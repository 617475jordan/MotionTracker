...
{
	staticimg = 0;
	String insert = "mine.mp4";			// Path to video that needs to be detected
	String output = "output.avi";	// Path to where detected video to be saved - Background Subtractor
	String output2 = "output2.avi";	// Path to where detected video to be saved - Absolute Difference

	//GetStatic(insert);
	MotionTracker(output, output2);
	return 0;
}
...
void GetStatic(String insert) {
	...
	Sleep(2000);
	imwrite("NoObjects.png", NoObject);
	capSrc.release();
	destroyAllWindows();
}
...
*/
void MotionTracker( String output, String output2) {

	Mat window, main, main2, diff, NoObject, detail, detail2, dst;

	VideoCapture capSrc(0);
	...
	while (capSrc.read(main)) {
		...
		if (staticimg.empty()) {
			staticimg = imread("test.png");
			resize(staticimg, staticimg, Size(dWidth, dHeight));
		}
		cout << staticimg.cols << " " << staticimg.rows << endl;
		...
		for (int k = 0; k < out.rows; k++)
		{
			for (int j = 0; j < out.cols; j++)
			{
				...
				{
					out.at<Vec3b>(k, j) = window.at<Vec3b>(k, j);
				}
				else
				{
					out.at<Vec3b>(k, j) = Vec3b(0, 0, 0);
				}
			}
		}
